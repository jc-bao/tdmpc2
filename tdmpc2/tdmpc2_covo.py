...
class TDMPC2CoVO:
    ...
    def plan(self, z, t0=False, eval_mode=False, task=None):
        ...
        for _ in range(self.cfg.iterations):
            # get value_mean hessian w.r.t. mean
            mean_grad = mean.detach().requires_grad_(True)
            value_mean_hess = torch.autograd.functional.hessian(
                partial(self._estimate_value, z[0], task=task), mean_grad
            ).squeeze()
            cost_hessian = -value_mean_hess

            # get covariance matrix
            cost_hessian_sym = (cost_hessian + cost_hessian.T) / 2
            eigns, u = torch.linalg.eigh(cost_hessian_sym)
            offset = torch.clip(-eigns.min() + 1e-2, 0, torch.inf)
            eigns = eigns + offset
            log_eigns = torch.log(eigns)
            element_num = log_eigns.shape[0]
            log_det_a_cov = element_num * np.log(self.cfg.max_std) * 2
            log_const = (log_det_a_cov * 2 + torch.sum(log_eigns)) / element_num
            log_eigns_a_cov = 0.5 * log_const - 0.5 * log_eigns
            # a_cov = u @ torch.diag(torch.exp(log_eigns_a_cov)) @ u.T
            a_cov_chol = u @ torch.diag(torch.exp(log_eigns_a_cov / 2))

            # sample from gaussian distribution with mean and a_cov
            noise = torch.randn(
                self.cfg.num_samples - self.cfg.num_pi_trajs,
                self.cfg.action_dim * self.cfg.horizon,
                device=std.device,
            )
            transformed_noise = a_cov_chol @ noise.T #  Tensor(action_dim * horizon, num_samples - num_pi_trajs)
            transformed_noise = transformed_noise.reshape(
                self.cfg.horizon, self.cfg.action_dim, self.cfg.num_samples - self.cfg.num_pi_trajs
            )
            transformed_noise = transformed_noise.permute(0, 2, 1)
            actions[:, self.cfg.num_pi_trajs :] = (
                mean.unsqueeze(1) + transformed_noise
            ).clamp(-1, 1)
